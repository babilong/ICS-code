...
for mn in range(len(MN)):
    ...
    for i in range(0, 11):
        ...
        #     M = 1
        # M = i
        f = open('./result/result_Real/multi/' + modelName + '/' + modelName + '_CR_{}.txt'.format(M))
        data = f.readlines()
        s1 = ''
    ...
    plt.title('Multiclassification Results of ' + modelName + ' Model', fontsize=14)
    ...
    plt.grid('on')
    plt.savefig('./result/result_Pic/line_chart/' + modelName + '/' + 'linechart_multi_' + modelName + '.jpg', dpi=600,
                bbox_inches='tight')
    plt.show()
