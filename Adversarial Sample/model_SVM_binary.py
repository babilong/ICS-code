...
# read data
# data1 = pd.read_csv('data_ICS_6_TT_binary.csv')
# data2 = pd.read_csv('data_ICS_6_TT_binary.csv')
multiple = 0

data1 = pd.read_csv('./train_test/data_ICS_6_TT_binary_train_shuffle.csv')
data2 = pd.read_csv('./train_test/data_ICS_6_TT_binary_test_shuffle.csv')
...
print(classification_report(y_test2, y_pred, target_names=['benign', 'vicious']))
with open('./ans/' + modelName + '_CR_{}.txt'.format(multiple), 'a') as f:
    f.write(classification_report(y_test2, y_pred, target_names=['benign', 'vicious']))
...
tpr = dict()
roc_auc = dict()
y_test2_o = to_categorical(y_test2)
y_pred_o = to_categorical(y_pred)
for i in range(labels.shape[1]):
    fpr[i], tpr[i], _ = roc_curve(y_test2_o[:, i], y_pred_o[:, i])
...
with open('./savemodel/binary/PSO-SVM.pickle', 'wb') as f:
    pickle.dump(clf, f)
