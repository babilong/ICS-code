...
# test
with open('./savemodel/multi/PSO-SVM.pickle', 'rb') as f:
    clf = pickle.load(f)
...
for multiple in range(0, 11):
    ...
    # M = multiple
    data_test = pd.read_csv('dataset/multi-test/data_ICS_6_TT_multi_noise_{}.csv'.format(M))

    labels_full_test = data_test['type']
    ...
    print(classification_report(y_test2, y_pred, target_names=['benign', 'SYN', 'UDP', 'TCP']))
    with open('./ans/' + modelName + '_CR_{}.txt'.format(M), 'a') as f:
        f.write(classification_report(y_test2, y_pred, target_names=['benign', 'SYN', 'UDP', 'TCP']))
